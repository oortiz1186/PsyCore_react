...
export default function PatientRecordPage() {
  ...
  function openTab(nextTab: TabKey) {
    setTab(nextTab);
    if (nextTab === 'history') setRefreshVersion(current => current + 1);
  }
  ...
  return <>
    ...
    <section className="patient-record-hero">
      ...
      <div className="patient-record-actions"><Link className="btn btn-secondary" href={`/appointments?patient=${patient.id}`}>Agendar cita</Link><button className="btn btn-primary" onClick={() => openTab('notes')}>Nueva nota SOAP</button></div>
    </section>
    ...
    <section className="dashboard-stats" aria-label="Resumen del expediente">
      ...
      <button className="metric-card" type="button" onClick={() => openTab('goals')}><span className="metric-icon"><Target size={21}/></span><span><small>Objetivos terapéuticos</small><strong>Abrir</strong></span></button>
    </section>

...
export async function POST(req:NextRequest){
 try{
  ...
  await admin.from('audit_events').insert({actor_id:user.id,action:'billing_item.created_from_appointment',entity_type:'billing_item',entity_id:String(item.id),patient_id:appointment.patient_id,metadata:{appointment_id:appointmentId,service_rate_id:rateId,source:'appointment_auto'}});

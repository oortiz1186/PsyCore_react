...
import { CalendarDays, CheckCircle2, ClipboardCheck, FileSignature, Send, UserRound } from 'lucide-react';
...
export default function PatientPortalPage() {
  ...
  return <main style={{ maxWidth: 1100, margin: '34px auto 70px', padding: '0 20px' }}>
    ...
    <div className="record-grid" style={{ marginTop: 22 }}><section className="card record-main-card"><div className="section-heading"><div><span className="eyebrow">Agenda</span><h2>Próximas citas</h2></div></div>{data.appointments.length ? <div className="appointment-list">{data.appointments.map(item => <div className="appointment-item" key={item.id}><span className="appointment-dot"/><div><strong>{item.starts_at ? new Intl.DateTimeFormat('es-MX',{dateStyle:'long',timeStyle:'short'}).format(new Date(item.starts_at)) : 'Fecha pendiente'}</strong><small>{item.consultation_mode || 'Consulta psicológica'}</small></div><span className="soft-chip">{item.status || 'Programada'}</span></div>)}</div> : <div className="empty-state compact-empty">No tienes citas próximas registradas.</div>}</section><aside className="card record-side-card"><span className="eyebrow">Acceso</span><h2>Invitación activa</h2><p className="muted">Este enlace es personal. No lo compartas.</p><small>Válido hasta</small><strong style={{display:'block',marginTop:6}}>{new Intl.DateTimeFormat('es-MX',{dateStyle:'long',timeStyle:'short'}).format(new Date(data.invite.expiresAt))}</strong></aside></div>

...
export function analyzePatient(input: ClinicalIntelligenceInput): ClinicalIntelligenceResult {
  ...
  const attendedAndCancelled = completedAppointments.length + cancelledAppointments.length;
  const adherence = attendedAndCancelled
    ? Math.round((completedAppointments.length / attendedAndCancelled) * 100)
    : null;
  ...
  const goalProgressValues = activeGoals
    .map(item => item.progress)
    .filter((value): value is number => typeof value === 'number');
  const averageGoalProgress = goalProgressValues.length
  ...
  if (phq9.latestScore !== null) {
    if (phq9.latestScore >= 20) {
      ...
      recommendations.push('Revisar de forma prioritaria el resultado de PHQ-9 y documentar la valoración clínica.');
      riskScore += 3;
    ...
    if (phq9.trend === 'up' && (phq9.delta || 0) >= 5) {
      ...
      recommendations.push('Explorar los cambios recientes asociados al aumento de síntomas depresivos.');
      riskScore += 2;
  ...
  if (gad7.latestScore !== null) {
    ...
    if (gad7.trend === 'up' && (gad7.delta || 0) >= 4) {
      ...
      recommendations.push('Explorar detonantes recientes y adherencia a las estrategias de regulación.');
      riskScore += 1;
  ...
  if (inactivityDays !== null && inactivityDays >= 45) {
    ...
    recommendations.push('Contactar al paciente o revisar si corresponde actualizar su estado de seguimiento.');
    riskScore += 2;
